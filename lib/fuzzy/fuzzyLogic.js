...
// At peak point, returns exactly 1
// At boundaries, returns 0
function triangularMembership(x, min, peak, max) {
  if (x < min || x > max) return 0;
  if (x <= peak) return (x - min) / (peak - min);  // Ascending slope
  return (max - x) / (max - peak);             // Descending slope
}
...
function evaluateRules(memberships) {
  ...
  for (const rule of rules) {
    const conditionResult = rule.conditions(memberships);
    const strength = conditionResult ? 1 : 0;
    if (strength > 0) {
...
function defuzzify(activeRules) {
  ...
  let totalWeight = 0;
  let feedingRateWeight = 0;  // Track weight separately for feeding_rate rules
  let adjustmentSum = 0;
  let feedingRateSum = 0;
  let hasFeedingRateRule = false;

  for (const rule of activeRules) {
    totalWeight += rule.strength;
    adjustmentSum += (rule.adjustment ?? 0) * rule.strength;
    if (rule.feeding_rate !== undefined) {
      hasFeedingRateRule = true;
      feedingRateWeight += rule.strength;  // Track weight of feeding_rate rules
      feedingRateSum += (rule.feeding_rate ?? 0) * rule.strength;
    }
  }

  const adjustment = totalWeight > 0 ? adjustmentSum / totalWeight : 1.0;
  // Use feedingRateWeight (only rules with feeding_rate) instead of totalWeight
  const feeding_rate = hasFeedingRateRule ? (feedingRateSum / feedingRateWeight) : 0.04;
